...
def gen(prompt, temperature, nsample):
    ...
    while len(all_choices) < nsample:
        ...
        try:
            ...
            cnt += 1
            time.sleep(5)
            print(f"Call failed (retry {cnt}): {e}")
...
def main():
    ...
    parser.add_argument(
        "--dataset-path",
        default='../dataset/xcodeeval/code_translation/xcodeeval_codetranslation_test_filtered.jsonl',
        help="dataset path.",
    )
    parser.add_argument(
        "--incremental",
    ...
    init_out_dir = f"{args.output_dir}/xcodeeval/{args.task}/initial-output"
    
    if not os.path.exists(init_out_dir):
        os.makedirs(init_out_dir, exist_ok=True)
    templates = [
        Template(
            f"code_translation_{idx}", template, "xCodeEval"
        )
        for idx, template in enumerate(xcodeeval_prompt_template["code_translation"])
    ]
    template = templates[0]

    code_translation_dataset = datasets.load_dataset('json', data_files={'test': args.dataset_path})['test']
    # code_translation_dataset = code_translation_dataset.select(range(2))
    ...
    existing_uids = set()
    if incremental:
        existing_uids = collect_existing_uids(init_out_dir)
        print(f"Incremental mode ON. Found {len(existing_uids)} existing generated samples in {init_out_dir}")
