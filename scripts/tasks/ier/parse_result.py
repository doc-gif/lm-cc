...
if __name__ == '__main__':
    ...
    der = args.der
    pl = args.pl
    filter_simple = args.filter
    task_idx = args.task_idx

    # result_root = "../Experiment_Results"
    result_root = "../output/humaneval-ier/initial-output"
    if not der:
        if dataset in ['CodeNet', 'Avatar']:
            ...
            result_dir = os.path.join(result_root, f"{dataset}-{task_idx}")
    else:
        if dataset in ['CodeNet', 'Avatar']:
    ...
    result_output_path = os.path.join(result_root, f"result-{dataset}-{task_idx}.json")
    ...
    if "gpt-3.5" in model or "gpt-4" or "deepseek-v3" in model:
    ...
    content = {
        ...
        "id_incorrect": id_incorrect,
    }

    # root = "../dataset"
    # if filter_simple and dataset == "humaneval":
    #     simple_dataset_path = os.path.join(root, "humaneval_simple")
    #     entries = list(os.listdir(simple_dataset_path))
    #     id_correct = [cid for cid in id_correct if cid in entries]
    #     id_incorrect = [iid for iid in id_incorrect if iid in entries]
    #     content = {
    #         "id_correct": id_correct,
    #         "id_incorrect": id_incorrect,
    #     }
    ...
    #     num_correct = len(id_correct)
    #     num_total = (len(id_correct) + len(id_incorrect))
    #     print("filtered:\ncorrect:", num_correct, "total:", num_total,  "accuracy:", num_correct/num_total)
